...
class HtmlDomParserLocal
{
    ...
    public function __construct(string $html, int $status, array $headers, string $text,
                                array $furtherInformation, UrlInfo $urlInfo,
...
//            $urlInfo->setUrl($redirect[$counter-1]);
//        }
        $counter = count($redirect);
        if($counter>0) {
    ...
    private function extractImageAttribute()
    {
        ...
        if(!is_bool($this->HtmlDomParser)) {
            foreach ($this->HtmlDomParser->find('img') as $e) {
                ...
                $value = UrlInfo::convertToUTF8($value);
                $value = UrlInfo::completeEditUrl($value, $this->url, $this->baseTag);
        ...
        if(!is_bool($this->HtmlDomParser)) {
            foreach ($this->HtmlDomParser->find('a') as $e) {
                ...
                if($value) {
                    ...
                    $text = $e->plaintext;
                    $url = UrlInfo::createUrlInfo($this->homeAddress, $value, $this->url, $text, $realUrl, ($this->depth + 1));
    ...
    private function checkUrl(string $href)
    {
        ...
        $href= UrlInfo::convertToUTF8($href);
        return $href;
    }
